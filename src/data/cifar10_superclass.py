...
def CIFAR10_SuperClass(train_ds, test_ds, args):
    ...
    for r, clust in enumerate(superclass):
        ...
        for c in clust:
            temp_idxs = np.hstack([temp_idxs, idxs_train[c*5000:(c+1)*5000]])
...
def CIFAR10_SuperClass_NIID(train_ds, test_ds, nlabels, args):
...
def CIFAR10_SuperClass_NIID_DIR(train_ds, test_ds, args):
...
def CIFAR10_SuperClass_Old_NIID(train_ds, test_ds, nlabels, args):
    ...
    sc2 = np.random.choice(classes, size=6, replace=False).tolist()
    superclass = [sc1, sc2]
...
def CIFAR10_SuperClass_Old_NIID_DIR(train_ds, test_ds, args):
