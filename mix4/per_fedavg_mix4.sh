...
#SBATCH --gres=gpu:1
#SBATCH --job-name=per_fedavg_mix4_homo_5ep
#SBATCH --err=results/per_fedavg_mix4_homo_5ep.err
#SBATCH --out=results/per_fedavg_mix4_homo_5ep.out
...
do
    dir='../save_results/per_fedavg/homo/mix4'
    if [ ! -e $dir ]; then
    mkdir -p $dir
    fi 
    
    python ./main_per_fedavg_mix4.py --trial=$trial \
    --rounds=50 \
    --num_users=100 \
    --frac=0.1 \
    --local_ep=5 \
